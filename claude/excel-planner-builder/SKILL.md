...
The argument describes the tracker or planner to build. If no argument is provided, ask the user to describe what they want to track.
...
Load `references/excel-standards.md` for the complete toolkit (palette, helpers, layout rules).
...
PUR_LINK_BG    = "EDD9FF"             # Saturday + task highlight
...
Load `references/gas-patterns.md` for all pattern implementations.
...
- **Batch vs individual writes**: use `setValues()/setBackgrounds()/setFontWeights()` batch calls when NOT merging; use individual range calls per run when merging is needed
- **Scheduled functions** must not call `SpreadsheetApp.getUi()` — use `Logger.log()` instead
- **Simple triggers** (`onEdit`) can only access the same spreadsheet; they cannot call external services. Use installable triggers for anything requiring authorization.
